...
class LdfModel(BaseEstimator):
    ...
    def __init__(self, **kwargs):
        ...
        default_kwargs = {
            ...
            'fold_num_test': 1,
            'fold_num_val': 1,
            'load_json': f'{os.environ.get("PROJECT_ROOT")}/out/ldf_ts/MLP_X3LARGE_CONS_20221118031509_t8_v20_do10_nxTrue_pcTrue/model.json',
            'load_weights': f'{os.environ.get("PROJECT_ROOT")}/out/ldf_ts/MLP_X3LARGE_CONS_20221118031509_t8_v20_do10_nxTrue_pcTrue/model.h5',
            'output_dir': os.path.join('out', 'bademo'),
            'train_epochs': 10000,
            'es_patience': 200,
            'n_rand_restarts': 1,
            'verbose': 2,
    ...
    def define_distn(self):
        ...
        if self.error_metric == 'zig':
            from ldf.models.distn_zig import DistnZIG
            self.distn = DistnZIG(self.params)
